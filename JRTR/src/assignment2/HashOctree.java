...
public class HashOctree {
	...
	/**
	 * First step in the hashoctree creation. This initializes the 
	...
	public HashOctreeCell getNbr_v2c(HashOctreeVertex v, int nbr_Obxyz) {
		...
		// $$ corresponds to the parameter nbr_0bxyz = 0b111
		// -------
		// |  |$$|
		// ---v---
		// |  |  |
		...
		//so the seeked cell can be computed by nbrCodeMinus plus appropriate bit magic.
		code = MortonCodes.nbrCodeMinus(code, v.maxLvl, 0b111 & (~ nbr_Obxyz));
		
		return getNextValidCell(code);
	...
	public HashOctreeCell getNbr_c2c(HashOctreeCell cell, int Obxyz){
		...
		return getNextValidCell(nbrCode);
	}
	...
	/**
	 * Traverse the hashes bottom up, starting with the hash provided,
	 *  until a cell is found which 
	 * has a morton code logically describing a parent of 'hash'.
	 * @param hash
	 * @return
	 */
	private HashOctreeCell getNextValidCell(long hash){
		if(hash < 0){
			return null;
		}
		assert(hash >0);
		while(getCell(hash) == null && hash > 0){
			hash = hash >>3;
		}
		
		assert(hash >=0);
		//if hash is = 0 no cell was found and the following method returns null.
		return getCell(hash);
	}
